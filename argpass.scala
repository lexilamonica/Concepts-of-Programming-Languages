...
object argpass {
  ...
  /* EXERCISE 2: complete the following higher-order function.
    ...
    It will only do it after the third call to f: at that point it updates all three instances of
      RefInt that it has received as arguments so far.
    Your code must return a tuple of the three integers provided by f in the order that they came back
      from calls, i.e., the integer from the first call to f is the first integer in the returned tuple.
 */
  ...
  // Your code must increment (add 1 to) the RefInt it receives and
  // return double the original value (stored in a separate RefInt instance) as the result.
